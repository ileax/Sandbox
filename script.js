class SandboxGame {
    constructor(gridSize = 16, cellSize = 40) {
        ...
        this.palette = [
            ...
            { name: 'repeater', color: '#c89664', darkColor: '#966432' },
        ...
        this.burnedLamps = {};
        this.fireDuration = 50000 + Math.random() * 100000;
    ...
    async loadTextures() {
        const textureFiles = {
            ...
            'repeater_right': 'Assets/repeater_right.png',
            'repeater_down': 'Assets/repeater_down.png',
            'repeater_left': 'Assets/repeater_left.png',
            'repeater_up': 'Assets/repeater_up.png',
    ...
    canConnectToRepeater(repeaterRow, repeaterCol, fromDirection) {
        const key = `${repeaterRow},${repeaterCol}`;
        const cellType = this.grid[repeaterRow][repeaterCol];
        let dir;
        
        if (cellType === 'repeater') {
            dir = this.repeaterStates[key] || 0;
    ...
    isRepeaterPowered(row, col) {
        const key = `${row},${col}`;
        const cellType = this.grid[row][col];
        
        if (cellType === 'repeater') {
            if (!(key in this.repeaterStates)) return false;
            const direction = this.repeaterStates[key];
    ...
    checkInputPowered(row, col, direction) {
        ...
        if (inputCell === 'repeater' || inputCell === 'inverter') {
            const inputKey = `${inputRow},${inputCol}`;
            let inputDir;
            if (inputCell === 'repeater') {
                inputDir = this.repeaterStates[inputKey];
            } else {
            ...
            if (inputDir !== undefined) {
                ...
                if (outRow === row && outCol === col) {
                    return this.isRepeaterPowered(inputRow, inputCol);
                }
            }
        }
    ...
    updateCursor(e) {
        ...
        if (cellType === 'switch' || cellType === 'repeater' || cellType === 'inverter') {
    ...
    handleMouseDown(e) {
        ...
        if (e.button === 0) {
            ...
            if (this.selectedColor === 'repeater') {
                const key = `${row},${col}`;
                if (!(key in this.burnedLamps)) {
                    this.grid[row][col] = 'repeater';
                    this.placingRepeater = true;
                    this.repeaterPlacePos = { row, col };
                    const direction = this.autoDetermineDirection(row, col, 'repeater');
                    this.repeaterStates[key] = direction;
                    this.canvas.style.cursor = 'pointer';
                }
    ...
    handleMouseMove(e) {
        ...
        if (this.placingRepeater && this.repeaterPlacePos && !this.isRightMouseDown) {
            this.handleDirectionSelection(this.repeaterPlacePos, 'repeater', e);
            return;
        }
    ...
    handleDirectionSelection(placePos, type, e) {
        ...
        if (x >= 0 && x < this.width && y >= 0 && y < this.height) {
            ...
            if (row >= 0 && row < this.gridSize && col >= 0 && col < this.gridSize) {
                ...
                if (newDir !== null) {
                    if (type === 'repeater') {
                        this.repeaterStates[key] = newDir;
    ...
    handleLeftClick(row, col) {
        ...
        if (cellType === 'repeater') {
            const currentDir = this.repeaterStates[key] || 0;
            this.repeaterStates[key] = (currentDir - 1 + 4) % 4;
            return;
        }
        ...
        if (this.selectedColor) {
            ...
            if (this.selectedColor === 'switch' || this.selectedColor === 'repeater' || this.selectedColor === 'inverter') {
                this.canvas.style.cursor = 'pointer';
            }
        }
    }
    ...
    handleRightClick(row, col) {
        ...
        } else if (cellType === 'repeater') {
            delete this.repeaterStates[key];
        } else if (cellType === 'inverter') {
    ...
    autoDetermineDirection(row, col, type) {
        ...
        if (row > 0 && (this.grid[row - 1][col] === 'repeater' || this.grid[row - 1][col] === 'inverter')) {
            const key = `${row - 1},${col}`;
            ...
            if (dir === 1) {
                if (this.energyGrid[row - 1][col] > 0 || this.isRepeaterPowered(row - 1, col)) {
                    activeDevices.push(dir);
                }
            }
        }

        if (col < this.gridSize - 1 && (this.grid[row][col + 1] === 'repeater' || this.grid[row][col + 1] === 'inverter')) {
            const key = `${row},${col + 1}`;
            ...
            if (dir === 2) {
                if (this.energyGrid[row][col + 1] > 0 || this.isRepeaterPowered(row, col + 1)) {
                    activeDevices.push(dir);
                }
            }
        }

        if (row < this.gridSize - 1 && (this.grid[row + 1][col] === 'repeater' || this.grid[row + 1][col] === 'inverter')) {
            const key = `${row + 1},${col}`;
            const cellType = this.grid[row + 1][col];
            let dir;
            if (cellType === 'repeater') {
        ...
        if (col > 0 && (this.grid[row][col - 1] === 'repeater' || this.grid[row][col - 1] === 'inverter')) {
            const key = `${row},${col - 1}`;
            const cellType = this.grid[row][col - 1];
            let dir;
            if (cellType === 'repeater') {
    ...
    updatePower() {
        ...
        while (changed && iterations < maxIterations) {
            ...
            for (const [key, direction] of Object.entries(this.repeaterStates)) {
                const [row, col] = key.split(',').map(Number);
                if (this.shouldDeviceActivate(row, col, direction, this.energyGrid, 'repeater')) {
    ...
    shouldDeviceActivate(row, col, direction, tempEnergyGrid, type) {
        ...
        if (inputCellType === 'repeater') {
            const inputDir = this.repeaterStates[`${inputRow},${inputCol}`] || 0;
            if (inputDir === direction) {
                return this.shouldDeviceActivate(inputRow, inputCol, inputDir, tempEnergyGrid, 'repeater');
            }
        }
    ...
    createFire(centerRow, centerCol, currentTime) {
        ...
        for (const [dr, dc] of directions) {
            ...
            if (newRow >= 0 && newRow < this.gridSize && newCol >= 0 && newCol < this.gridSize) {
                const key = `${newRow},${newCol}`;
                this.burnedLamps[key] = currentTime;
    ...
    getWireTexture(row, col) {
        ...
        if (row > 0) {
            const cellType = this.grid[row - 1][col];
            if (cellType === 'repeater' || cellType === 'inverter') {
                if (this.canConnectToRepeater(row - 1, col, 2)) neighbors[0] = true;
        ...
        if (col < this.gridSize - 1) {
            const cellType = this.grid[row][col + 1];
            if (cellType === 'repeater' || cellType === 'inverter') {
                if (this.canConnectToRepeater(row, col + 1, 3)) neighbors[1] = true;
            } else if (['blue', 'red', 'green', 'switch'].includes(cellType)) {
                neighbors[1] = true;
            }
        }
        ...
        if (row < this.gridSize - 1) {
            const cellType = this.grid[row + 1][col];
            if (cellType === 'repeater' || cellType === 'inverter') {
                if (this.canConnectToRepeater(row + 1, col, 0)) neighbors[2] = true;
            } else if (['blue', 'red', 'green', 'switch'].includes(cellType)) {
                neighbors[2] = true;
            }
        }
        ...
        if (col > 0) {
            const cellType = this.grid[row][col - 1];
            if (cellType === 'repeater' || cellType === 'inverter') {
                if (this.canConnectToRepeater(row, col - 1, 1)) neighbors[3] = true;
            } else if (['blue', 'red', 'green', 'switch'].includes(cellType)) {
                neighbors[3] = true;
            }
        }
    ...
    draw() {
        ...
        for (const [key, creationTime] of Object.entries(this.burnedLamps)) {
            if (currentTime - creationTime > this.fireDuration) {
                const [row, col] = key.split(',').map(Number);
                ...
                if (key in this.burnedLamps) {
                    ...
                    if (cellType) {
                        ...
                        if (cellType === 'red' && this.textures['psu']) {
                            ...
                            const direction = this.repeaterStates[key] || 0;
                            const dirNames = ['right', 'down', 'left', 'up'];
                            const textureName = `repeater_${dirNames[direction]}`;
